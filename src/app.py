...
radar_cities.update_layout(title='', title_y=0.97,
                               ...
                               coloraxis_showscale=True, template='plotly_dark')
radar_cities.show()
...
fig_map.update_layout(title='',
                   width=480, height=500, margin={'r':0, 'l':0, 'b':0, 'pad':0}, template='plotly_dark')
...
app = dash.Dash(__name__, external_stylesheets=[dbc.themes.CYBORG])  # Use the CYBORG theme instead of BOOTSTRAP_DARK
server=app.server
header_style = {"backgroundColor": "#343a40",
                "padding": "1.5rem",
                "textAlign": "center",
                "color": "white"}
...
new_card = dbc.Card([
    ...
    ], style={"backgroundColor": "#343a40", "padding": "1rem", "height": "100%"}),
], style={"height": "480px"})
...
total_heading = dbc.Card(
    dbc.CardBody(
        [
            html.H1("Discovering the UK's Culinary Capitals", className="mb-0", style={"backgroundColor": "#343a40", "color": "#ffffff", "fontWeight": "bold", "padding": "1.5rem 0", "padding-left": "1em", "text-align": "left"}),
